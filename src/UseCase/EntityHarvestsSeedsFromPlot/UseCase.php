...
namespace ConorSmith\Hoarde\UseCase\EntityHarvestsSeedsFromPlot;
...
final class UseCase
{
    ...
    public function __invoke(
        ...
        if ($varietyId->equals(Uuid::fromString(VarietyRepositoryConfig::RADISH_PLANT))) {
            $harvestedVariety = $this->varietyRepository->find(Uuid::fromString(VarietyRepositoryConfig::RADISH_SEED));
        } else {
        ...
        $inventoryEntity->getInventory()->addItem(
            $harvestedVariety->createItemWithQuantity($quantity * 2)
        );
