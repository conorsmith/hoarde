...
use ConorSmith\Hoarde\UseCase\EntityTravels\UseCase;
use Psr\Http\Message\ResponseInterface;
use Psr\Http\Message\ServerRequestInterface;
use Ramsey\Uuid\Uuid;
use Zend\Diactoros\Response\RedirectResponse;

final class EntityTravels
{
    /** @var Segment */
    private $session;
    ...
    public function __invoke(ServerRequestInterface $request, array $args): ResponseInterface
    {
        ...
        if (!Direction::isValid($_POST['direction'])) {
            $this->session->setFlash("danger", "{$_POST['direction']} is not a valid direction.");
            $locationId = $this->findActorLocation->__invoke($gameId, $actorId);
            return new RedirectResponse("/{$gameId}/{$locationId}");
        }

        $direction = new Direction($_POST['direction']);

        $result = $this->useCase->__invoke($gameId, $actorId, $direction);
